...
\indent Today's economy, financial asset bubble is  exciting and hot topic. 
In most recent market news, we have read or seen sudden change in gold price. 
People are interested to know what will happen in the future. How someone can be millionaire by investing in stock market? How we will able to detect or estimate the future changes of any asset ( stock, gold, housing, commodity)? 
How quickly asset price will jump? These are the questions which we will consider in this thesis work. 
We will study how to determine whether any asset is experiencing a price bubble in a real time. 
%  Asset bubbles are an exciting and hot topic.  Asset bubbles are [in one senctence describe an asset bubble].  If the price of an asset increases, i.e. bubbles, well beyond intrinsic value, an investor would like to be able to detect the bubble to manager their investments with respect to that asset.  In this thesis, we discuss a method for estimating volatility of an  asset price and test for an asset bubble.  We then cover some examples using out method to test whether an asset price is experiencing a price bubble.  

\indent We estime the volatility of asset prices using the Florens-Zmirou estimator. Our goal is to estimate stock price volatility by Floren Zmirou estimator and then we will extrapolate the volatility tale in order to test if the price exhibits a martingale or supermartingale.%[citation needed here].
%
% 
%Using some helpful techniques to determine asset bubble in real time will help finanial corporations, banks,and money marrkets to save or earn more money.\\
...
% 
% \indent The process for bubble detection depends on a mathematical analysis that determines when an asset is undergoing speculative pricing
...
%  The biggest challenge which we faced using non parametric estimator is that we can only estimate $\sigma(x)$ volatility function on the points which are visisted by the process.
...
%vvvv---Everything below this comment needs a lot of work
\indent Our work is orginized as follow: in chapter 2 we present an overview of previous work, background of the problem, how the problem is connected to finance and mathematics, the methods to solve the problem, and 
our best possible solution to the problem. In chapter 3 we will discuss the details of our algorithm and it's implementation and in chapter 4 we present several numberical examples, conclusion and future work.
